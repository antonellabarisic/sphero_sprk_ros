...
class SpheroNode(object):
    ...
    def __init__(self, default_update_rate=50.0):
        ...
        self._address = rospy.get_param(self._node_name + "/address")
    ...
    def _init_pubsub(self):
        ...
        self.manual_calibration_sub= rospy.Subscriber(
            'manual_calibration', Bool, self.manual_calibration, queue_size=1)
    ...
    def _init_params(self):
        ...
        self.cmd_vel_timeout = rospy.Duration(
            rospy.get_param('~cmd_vel_timeout', 5.0))
        self.diag_update_rate = rospy.Duration(
            rospy.get_param('~diag_update_rate', 5.0))
...
# program
    def start(self):
        ...
        try:
            ...
            rospy.loginfo("Connected to Sphero %s" %
                          self._namespace + "\n with address: %s" % self._address)
            #set initial color of Sphero indicating connection
            self.robot.set_rgb_led(
                self.connect_color_red, self.connect_color_green, self.connect_color_blue, 1, False)
        except:
    ...
    def spin(self):
        """Loop"""
        r = rospy.Rate(10.0) #10.0
        while not rospy.is_shutdown():
            now = rospy.Time.now()
            if self._data_stream=='Locator':
                self.get_locator_data()

            # if there is no command for velocity longer than 5sec, stop the Sphero
            ...
            # update diagnostic info every 5sec
    ...
    def set_heading(self, msg):
        ...
        if self.is_connected:
            heading_deg = int(self.normalize_angle_positive(
                math.radians(msg.data)) * 180.0 / math.pi)
